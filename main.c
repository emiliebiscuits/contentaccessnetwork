...
void mettreAJourVoisins(Voisins *v, Espace *e, const int * const identifiant, const int size, const int rank)
{
	int i;
	int annonceVoisin[7];
	for(i=1;i<size;i++)
...
void suppression(Voisins *v, GSList *l, Espace *e, Donnees *d, int rank)
{
	...
	while(temp!=NULL && estValide(e))
	{
		...
		{
			reponse = 1;
			//Lui dire de preparer les transfers
			MPI_Send(&reponse, 1, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD);
			...
			//Attendre l'aquittement de l'espace
			MPI_Recv(NULL, 0, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			//Attendre l'aquittement de la taille des donnees
			MPI_Recv(NULL, 0, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			//Attendre l'aquittement des donnees
			MPI_Recv(NULL, 0, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			
			//Lui dire la taille des voisins stockés
			/*taille = tailleVoisins(v);
			MPI_Send(&taille, 1, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD);
			//Attendre l'aquittement de la taille des voisins
			MPI_Recv(NULL, 0, MPI_INT, ptr_i[0], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int main(int argc,char **argv)
{
	...
	mettreAJourVoisins(&v, &e, identifiant, size, rank);
	...
	if(rank == 0)
	{
		...
		while(temp != NULL)
		{
			...
			printf("Processus 0 a recu donnée : %d à partir une recherche de broadcast\n", sum);
			temp = temp->next;
		}
		coordonnee[0] = -1;
		coordonnee[1] = -1;
	...
	else
	{
		...
		while(arret == 0)
		{
			...
			else if(estDedans(&e,coordonnee[0],coordonnee[1]))
			{
				...
				printf("Processus %d a trouvé donnée (%d,%d) chez lui\n",rank,coordonnee[0],coordonnee[1]);
				//Envoyer la reponse
	...
	if(rank == 0)
	{
		...
		while(temp != NULL)
		{
			...
			MPI_Recv(&sum, 1, MPI_INT,MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			printf("Processus a recu la donnee : %d à partir d'une recherche proche en proche\n", sum);
			temp = temp->next;
		}
		coordonnee[0] = -1;
		coordonnee[1] = -1;
	...
	else
	{
		...
		while(arret != 1)
		{
			...
			else if(estDedans(&e,coordonnee[0],coordonnee[1]))
			{
				//Parcourir les donnees et trouver l'element recherché
				sum = trouverValeur(&d,coordonnee[0],coordonnee[1]);
				printf("Processus %d a trouvé (%d,%d) chez lui\n",rank,coordonnee[0],coordonnee[1]);
				//Envoyer la reponse
				MPI_Send(&sum, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
			}
			...
			{
				i = trouverProche(&v, coordonnee[0],coordonnee[1], e.xdebut, e.xfin, e.ydebut, e.yfin);
				printf("Processus %d a recu (%d,%d), il a envoyé à processus %d\n",rank,coordonnee[0],coordonnee[1],i);
				MPI_Send(&coordonnee, 2, MPI_INT, i, 0, MPI_COMM_WORLD);
			}
		}
	}
	...
	if(rank==0)
	{
		...
		for(i=1;i<size;i++)
		{
			MPI_Send(&supp, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
		}
	}
	else
	{	
			...
			while(arret != 1)
			{
				...
				else
				{
					...
					if(reponse != 0)
					{
						...
						mettreAJourEspace(&e,espaceRecu);
						...
						ajouterDeupuisTab(&d,donneesTransfer,taille);
						free(donneesTransfer);
	...
	if(rank!=0)
	{
		printf("Espace de processus %d: ",rank);
		affichageEspace(&e);
	}
	MPI_Barrier(MPI_COMM_WORLD);
